...
class IrAttachment(models.Model):
    ...
    def _set_folder_settings(self, vals):
        ...
        if vals.get('res_model') == 'sale.order' \
                and self.env.user.company_id.dms_sale_settings \
                and not vals.get('folder_id'):
            folder = self.env.user.company_id.sale_folder
            if folder.exists():
                vals.setdefault('folder_id', folder.id)
                vals.setdefault('tag_ids', [(6, 0, self.env.user.company_id.sale_tags.ids)])
        return vals
