odoo.define('rds_mail.rds_chat_client_action', function (require) {
"use strict";

var chat_manager = require('rds_mail.rds_chat_manager');
...
var ChatAction = Widget.extend(ControlPanelMixin, {
    ...
    events: {
        ...
        'click .or_thread_item': '_onThreadClick',
        'click .or_thread_checkbox': '_onThreadSelect',
        'click .or_load_more': '_onLoadMore',
        'click .btn-fav': '_onFavToggle',
        'keydown': '_onKeydown',
    },
    ...
    start: function () {
        ...
        var default_channel = chat_manager.get_channel(this.defaultChannelID) ||
                                chat_manager.get_channel('channel_threads_inbox');
    ...
    _fetchAndRenderThread: function () {
        var self = this;
        var is_thread_ch = false
        try {
            is_thread_ch = (this.channel.id.indexOf('threads') != -1);  
        } catch(err) {}
        
        return chat_manager.get_messages({channel_id: this.channel.id, is_thread_ch: is_thread_ch, domain: this.domain}).then(function (result) {
            if (is_thread_ch == true) {
                self.nomore = result[1]
                self._renderThreads(result[0], self.thread.$el);
            } else {
     ...
     * @private
     */        
    _renderThreads: function (threads, $el, extend=false) {
        if (extend) {
        ...
        for (var i = 0; i < Object.keys(threads).length; i++) {
            $el.append($(QWeb.render("rds_mail.ThreadListItem", threads[i])));
            this.cached.push([threads[i].model, threads[i].id])
        }
        this.$loadmore = $(QWeb.render("rds_mail.LoadMoreThreads", threads[i]))
        $el.append(this.$loadmore)
    ...
    _renderButtons: function () {
        ...
        this.$buttons.on('click', '.o_mail_chat_button_new', this._onNewClicked.bind(this));
        this.$buttons.on('click', '.o_mail_chat_button_mass_hide', this._onMassHideClicked.bind(this));
        this.$buttons.on('click', '.o_mail_chat_button_mass_uf', this._onMassUnfollowClicked.bind(this));
        this.$buttons.on('click', '.o_mail_chat_button_mark_read_rds', this._onMarkAllReadRdsClicked.bind(this));
    },
    /**
     * @private
     * @returns {Deferred}
    ...
    _updateControlPanelButtons: function (channel) {
        ...
        this.$buttons
            ...
            .removeClass("o_hidden");
        this.$buttons
            .find('.o_mail_chat_button_mark_read_rds')
            .toggle((channel.id === "channel_threads_inbox") || (channel.id === "channel_threads_starred"))
            .removeClass("o_hidden");
        this.$buttons
            .find('.o_mail_chat_button_mass_hide')
            .toggle((channel.id === "channel_threads_inbox") || (channel.id === "channel_threads_starred"))
            .removeClass("o_hidden");
        this.$buttons
            .find('.o_mail_chat_button_mass_uf')
            .toggle(channel.id === "channel_threads_silenced")
            .removeClass("o_hidden");
     ...
     * @private
     */
    _onMarkAllReadRdsClicked: function (event) {
        var records = []
        this.$('.t_selected').each(function() {records.push([$( this ).attr('data-model'), $( this ).attr('data-id')])})
        this._rpc({
            model: 'mail.message',
            method: 'threads_mark_read',
            args: [records],
        })
        this.ButtonsReset();
    },
     ...
     * @private
     */
    _onMassHideClicked: function (event) {
        var records = []
        this.$('.t_selected').each(function() {records.push([$( this ).attr('data-model'), $( this ).attr('data-id')]); this.remove();})
        this._rpc({
            model: 'mail.message',
            method: 'threads_silence',
            args: [records],
        })
        this.ButtonsReset();
    },
     ...
     * @private
     */
    _onMassUnfollowClicked: function(event) {
        var records = []
        this.$('.t_selected').each(function() {records.push([$( this ).attr('data-model'), $( this ).attr('data-id')]); this.remove();})
        this._rpc({
            model: 'mail.message',
            method: 'threads_unfollow',
            args: [records],
        })
        this.ButtonsReset();
    },
    ...
    _onNewClicked: function (event) {
        this.do_action({
            ...
            views: [[false, 'form']],
            target: 'new',
            context: {create_new_topic: true}
        });
    },
    /**
     * @private
     */        
    _onUnfollowClicked: function (event) {
        return
    },
    /**
     * @private
     */        
    _onThreadClick: function (event) {
        event.preventDefault();
        var $target = $(event.target);
        $target.removeClass('unread-1')
        this.do_action({
            type: 'ir.actions.act_window',
            view_type: 'form',
            view_mode: 'form',
            res_model: $target.data('model'),
            views: [[false, 'form']],
    ...
    ButtonsReset: function () {
        this.$buttons.find(".o_mail_chat_button_mark_read_rds").addClass("rds_mail_noshow")
        this.$buttons.find(".o_mail_chat_button_mass_uf").addClass("rds_mail_noshow")
        this.$buttons.find(".o_mail_chat_button_mass_hide").addClass("rds_mail_noshow")
    },

    /**
     * @private
     */        
    _onThreadSelect: function (event) {
        event.stopPropagation();
        var $target = $(event.target);

        var $thread = $target.closest(".or_thread_item")
        $thread.toggleClass("t_selected")

        if (this.$(".t_selected").length == 0) {
            this.$buttons.find(".o_mail_chat_button_mark_read_rds").addClass("rds_mail_noshow")
            this.$buttons.find(".o_mail_chat_button_mass_uf").addClass("rds_mail_noshow")
            this.$buttons.find(".o_mail_chat_button_mass_hide").addClass("rds_mail_noshow")
        } else {
            this.$buttons.find(".o_mail_chat_button_mark_read_rds").removeClass("rds_mail_noshow")
            
            if (this.channel.id == 'channel_threads_silenced')
                this.$buttons.find(".o_mail_chat_button_mass_uf").removeClass("rds_mail_noshow")

            if ((this.channel.id == 'channel_threads_inbox') || (this.channel.id == 'channel_threads_starred'))
                this.$buttons.find(".o_mail_chat_button_mass_hide").removeClass("rds_mail_noshow")
        }
    },
    /**
     * @private
     */        
    _onFavToggle: function (event) {
        event.stopPropagation();
        var $target = $(event.target);
        var $thread = $target.closest(".or_thread_item")

        var id = $thread.data("id")
        var model = $thread.data("model")
        ...
        this._rpc({
            ...
            $target.parent().removeClass('followstate-follow followstate-starred')
            $target.parent().addClass('followstate-' + result)
        });
    },
    /**
     * @private
    ...
    _onLoadMore: function (event) {
        ...
        chat_manager.get_messages({channel_id: this.channel.id, domain: this.domain, load_more: true, cached: this.cached}).then(function (result) {
            self._renderThreads(result[0], self.thread.$el, true);
        });
    },    
    /**
     * @private
...
core.action_registry.add('rds_mail.chat.instant_messaging', ChatAction);
