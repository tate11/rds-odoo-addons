odoo.define('rds_mail.rds_chat_manager', function (require) {
"use strict";
...
function make_message (data) {
    ...
    if (msg.model === 'mail.channel') {
        var real_channels = _.without(msg.channel_ids, 'channel_inbox', 'channel_starred','channel_threads_inbox','channel_threads_starred','channel_threads_silenced');
...
var ChatManager =  Class.extend(Mixins.EventDispatcherMixin, ServicesMixin, {
    ...
    start: function () {
        ...
        add_channel({
            id: "channel_threads_inbox",
            name: _lt("Threads Attivi"),
            type: "static"
        });

        add_channel({
            id: "channel_threads_starred",
            name: _lt("Threads Preferiti"),
            type: "static"
        });

        add_channel({
            id: "channel_threads_silenced",
            name: _lt("Thread Nascosti"),
            type: "static"
        });
    ...
    _fetchInbox: function (channel, options) {
        options = options || {};
        
        var fwstate = (channel.id === "channel_threads_inbox")   ? ['follow', 'starred'] :
                      (channel.id === "channel_threads_starred") ? ['starred']           :
                      (channel.id === "channel_threads_silenced") ? ['silenced']          :
                                                                   [[]];
        var _cached = []
        ...
        return this._rpc({
            ...
            method: 'thread_fetch',
            args: [domain],
            kwargs: {cached: _cached, fwstate: fwstate, context: session.user_context},
        })
    },
    ...
    get_messages: function (options) {
        var channel
        if ('channel_id' in options && (('is_thread_ch' in options) && (options.is_thread_ch == true)) && options.load_more) {
            // get channel messages, force load_more
            channel = this.get_channel(options.channel_id);
            return this._fetchInbox(channel, {domain: options.domain, cached: options.cached})

        }

        if ('channel_id' in options && (('is_thread_ch' in options) && (options.is_thread_ch == true))  ) {
            // get channel messages, force load_more
            channel = this.get_channel(options.channel_id);
            return this._fetchInbox(channel, {domain: options.domain})
