...
logfile = os.path.join(modulePath, 'chronojob_dia_product_product.log')
logging.basicConfig(filename=logfile,level=logging.DEBUG, format='%(asctime)s [%(levelname)s]%(name)s - %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p')
...
def callOdooFunction(odooObj, functionName, parameters=[], kwargParameters={}):
    ...
    '''
    try:
        return socketYesLogin.execute_kw(databaseName, userId, userPassword,
                                              odooObj,
                                              functionName,
                                              parameters,
                                              kwargParameters)
...
try:
    ...
    if userId:
        ...
        logging.info('Calling method cron_send_to_dia')
        try:
            res = callOdooFunction(odooObj='product.template', functionName='cron_send_to_dia')
            if not res:
