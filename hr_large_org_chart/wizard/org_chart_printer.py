    ...
    def get_employees_xml_tree(self):
        ...
        def build_element(e, xml_e):
            ...
            def build_extended_array(e):
                ...
                expanded_children = self.env['hr.employee'].search(['&', ('parent_id', '=', e.ids[0]), ('child_ids', '!=', False)]).sorted(key=lambda e: e.job_id.priority)
                
                collapsed_children = []

                for job in jobs:
                    employees_in_job = self.env['hr.employee'].search(['&', '&', ('parent_id', '=', e.ids[0]), ('job_id', '=', job.ids[0]), ('child_ids', '=', False)])
        ...
        root_employee = self.env['hr.employee'].search([('parent_id', '=', False)])[0]
